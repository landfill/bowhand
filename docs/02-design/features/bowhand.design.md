...
> **Summary**: 듀얼 카메라 AR 패스스루 + 핸드트래킹 3D 양궁 게임의 상세 설계
>
> **Project**: BowHand
> **Version**: 0.3.0
> **Author**: h0977
> **Date**: 2026-03-23
> **Status**: Updated (Act iteration 3 — AR architecture sync)
...
| **Solution** | 후면 카메라 AR 패스스루 + 전면 카메라 MediaPipe 핸드트래킹 + Three.js 투명 오버레이 |
| **Function/UX Effect** | 후면 카메라의 실제 환경 위에 3D 과녁이 떠 있고, 손 제스처로 활을 당겨 발사하는 AR 양궁 체험 |
| **Core Value** | 스마트폰 듀얼 카메라만으로 현실 공간에서 실감나는 AR 양궁 체험 |
...
│  │ Manager     │   │ + Gesture   │   │ (투명 렌더러)│        │
│  │ (듀얼카메라) │   │   Engine    │   │             │        │
│  └──────┬──────┘   └──────┬──────┘   └──────┬──────┘        │
│         │                  │                  │               │
│    ┌────┴────┐             ▼                  ▼               │
│    ▼         ▼      ┌─────────────┐   ┌─────────────┐        │
│ 후면cam   전면cam    │ Aim         │   │ Arrow       │        │
│ (AR배경)  (핸드트래킹)│ Controller  │   │ Physics     │        │
│    │         │      └─────────────┘   └─────────────┘        │
│    ▼         ▼             │                  │               │
│ ar-bg    pip-view          ▼                  ▼               │
│ video    video      ┌────────────────────────────┐            │
...
│                                                               │
│  렌더링 레이어 (z-index 순서):                                 │
│  [0] ar-background (후면 카메라 영상)                          │
│  [1] Three.js canvas (투명 배경, 3D 타겟+화살 오버레이)         │
│  [10] HUD (파워게이지, 상태, 점수)                              │
...
─────────────────────────────────────────────────────
0. ar-background <video> 자동 재생 (후면 카메라 실시간 영상 — 배경)

1. HandTracker.detect(frontVideo)
   → 전면 카메라 프레임으로 MediaPipe Hands 추론 (21 랜드마크)
   → GestureEngine.update(landmarks)
   → 이벤트 발행: 'release' (GestureEngine EventTarget)

2. AimController.update(handPosition, camera)
   → 손 위치 → NDC 좌표 → 3D 월드 좌표 변환
   → 크로스헤어 위치 갱신

3. ArrowPhysics.update(deltaTime)
   → (RELEASE 시) 화살 궤적 계산 + 이동
   → Raycasting 적중 판정
   → 이벤트 발행: 'hit' | 'miss' (ArrowPhysics EventTarget)

4. SceneManager.render()
   → Three.js 투명 캔버스 렌더링 (타겟 + 화살 + 크로스헤어만 — AR 오버레이)
...
|-----------|-----------|---------|
| HandTracker | CameraManager (전면) | 전면 카메라 비디오 프레임 입력 |
...
**책임**: 듀얼 카메라 스트림 획득 및 관리 (후면 AR 배경 + 전면 핸드트래킹)

```typescript
class CameraManager {
  private frontVideo: HTMLVideoElement;   // 전면 카메라 (핸드트래킹용)
  private rearVideo: HTMLVideoElement;    // 후면 카메라 (AR 배경용)
  private frontStream: MediaStream | null;
  private rearStream: MediaStream | null;
  private dualSupported: boolean;

  async init(): Promise<void>
  // 1. 후면 카메라: getUserMedia({ facingMode: { exact: 'environment' }, 1280x720 })
  // 2. 전면 카메라: getUserMedia({ facingMode: 'user', 640x480 })
  // 3. 후면 카메라 실패 시 (데스크톱): 전면 카메라로 양쪽 대체
  // 4. 카메라 권한 거부 시 CAMERA_DENIED 에러

  getVideoElement(): HTMLVideoElement
  // 전면 카메라 video — HandTracker에 전달

  getRearVideoElement(): HTMLVideoElement
  // 후면 카메라 video — ar-background에 연결
  ...
  destroy(): void
  // 양쪽 스트림 정리, 트랙 중지, video 엘리먼트 제거
}
```

**핵심 결정:**
- **듀얼 카메라**: 후면(`environment`, 1280x720)은 AR 배경, 전면(`user`, 640x480)은 핸드트래킹
- **폴백**: 후면 카메라 없는 환경(데스크톱)에서는 전면 카메라를 양쪽에 사용
- **비디오 엘리먼트**: `display: none`으로 숨기고, main.ts에서 ar-background/pip-view에 srcObject 연결
...
### 4.4 SceneManager (AR 투명 오버레이)

**책임**: Three.js 투명 캔버스로 AR 오버레이 렌더링 (배경은 후면 카메라 영상)
...
class SceneManager {
  ...
  // WebGLRenderer 초기화 (antialias: false, alpha: true — AR 투명 배경)
  // setClearColor(0x000000, 0) — 완전 투명
  // PerspectiveCamera (fov: 60, 1인칭 시점, position: (0, 1.6, 0))
  ...
  // 투명 배경 위에 3D 오브젝트만 렌더링 → ar-background 비디오 위에 오버레이
...
**FantasyField (미사용 — 향후 비AR 모드용 예비):**
- `src/scene/FantasyField.ts`에 클래스 정의는 존재하나, AR 모드에서는 사용하지 않음
- 후면 카메라 없는 데스크톱 환경에서 대체 배경으로 활용 가능

**핵심 결정:**
- **alpha: true**: 투명 WebGL 렌더러로 후면 카메라 영상 위에 3D 오버레이
- **antialias: false**: 모바일 성능 확보
- **pixelRatio 제한**: 고해상도 디바이스에서 과도한 렌더링 방지
- **AR 조명**: main.ts에서 DirectionalLight + AmbientLight 직접 추가
...
└────────────────────────────────────────────┘
  ...
  레이어 스택 (z-index):
  [0]  ar-background <video> — 후면 카메라 영상 (전체 화면, object-fit: cover)
  [1]  Three.js <canvas> — 투명 배경, 3D 타겟+화살+크로스헤어만 렌더링
  [10] HUD — 파워 게이지, 상태 텍스트, 점수 (HTML absolute)
  [20] pip-view <video> — 전면 카메라 미리보기 (120x90px, 우하단, 미러링)
```
...
| CameraManager | src/camera/ | 듀얼 카메라 스트림 획득 (후면 AR + 전면 핸드트래킹) |
| HandTracker | src/tracking/ | MediaPipe 핸드 추적 |
| GestureEngine | src/tracking/ | 제스처 상태 머신 |
| SceneManager | src/scene/ | Three.js 투명 오버레이 씬 관리 |
| FantasyField | src/scene/ | 판타지 환경 렌더링 (AR 모드에서 미사용) |
...
| 0.3 | 2026-03-23 | AR 아키텍처 피봇 반영: 듀얼 카메라 시스템, 투명 WebGL 렌더러, AR 패스스루 레이어 구조, FantasyField 미사용 명시, UI 레이아웃에 PiP 추가, iOS Safari play() 대응 | h0977 |
